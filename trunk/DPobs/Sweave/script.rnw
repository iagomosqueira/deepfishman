...
fit <- optim(SSB0,fn = logl,catch=catch(stk2)[,1:maxt],index=index[,1:maxt],hh=slope,M=m(gen1),mat=mat(gen1),sel=landings.sel(gen1),wght=stock.wt(gen1),amin=range(gen1)['min'],amax=range(gen1)['max'],method = "L-BFGS-B",lower = c(800),upper = c(1500),hessian = T)
out<-pdyn(B0=fit$par,catch=catch(stk2)[,1:maxt],index=index[,1:maxt],hh=slope,M=m(gen1),mat=mat(gen1),sel=landings.sel(gen1),wght=stock.wt(gen1),amin=range(gen1)['min'],amax=range(gen1)['max'])
#plot(as.vector(index)[1:40])
#lines(out$Ipred)

msy <- msy.sra(B0=fit$par,catch=catch(stk2)[,1:maxt],index=index[,1:maxt],hh=slope,M=m(gen1),mat=mat(gen1),sel=landings.sel(gen1),wght=stock.wt(gen1),amin=range(gen1)['min'],amax=range(gen1)['max'])
CTAR <- msy$MSY
BTAR <- msy$MSY/msy$F
@
...
hcr <- function(catch,index,year) {
  ...
  fit <- optim(SSB0,fn = logl,catch=catch,index=index,hh=slope,M=m(gen1),mat=mat(gen1),sel=landings.sel(gen1),wght=stock.wt(gen1),amin=range(gen1)['min'],amax=range(gen1)['max'],method = "L-BFGS-B",lower = c(800),upper = c(1500),hessian = T)
  out<-pdyn(B0=fit$par,catch=catch,index=index,hh=slope,M=m(gen1),mat=mat(gen1),sel=landings.sel(gen1),wght=stock.wt(gen1),amin=range(gen1)['min'],amax=range(gen1)['max'])
...
index <- quantSums(sweep(stock.n(stk2) * stock.wt(stk2),1,catch.sel(gen1),"*")) * q.surv
for(y in (proj_strt+1):proj_end) {

  # control rule
  ctrl <- hcr(catch(stk2)[,1:(y-1)],index[,1:(y-1)],y)
  # project
  stk2 <- fwd(stk2,ctrl=ctrl,sr=list(model=model(gen1), params=params(gen1)), sr.residuals=sr_resid)
  # update survey index
  index[,y] <- quantSums(stock.n(stk2)[,y] * stock.wt(stk2)[,y] * catch.sel(gen1)) * q.surv
}
@
...
print(ggplot(data.frame(Time=1:proj_end, B=c(quantSums(sweep(stk2@stock.n * stk2@stock.wt,1,catch.sel(gen1),"*"))))) 
  + geom_line(aes(x=Time,y=B)) 
  + geom_line(aes(x=Time,y=BMSY),data=data.frame(Time=1:proj_end, BMSY=BTAR))
  + geom_line(aes(x=proj_strt,y=B))
  )
@
