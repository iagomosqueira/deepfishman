...
v <- 1e3
# Make the stock
gen1 <- genBRP(age=1:maxage, Linf=Linf, k=k, a1=a1, sL=sL, sR=sR, mat95=mat95, s=s, v=v)
@    
...
before decreasing slightly over a period of \Sexpr{maxt} years. %We'll also multiply the $F$ value by lognormally distributed noise with a mean of 1 and a standard deviation of \Sexpr{Fnoise_sd}. 
...
scenario. %We'll put a small amount of lognormally distributed noise onto the recruitment with a mean of 1 and a standard deviation of \Sexpr{srr_sd}.
...
index <- apply(sweep(stk2@stock.n * exp(-stk2@m/2) * stk2@stock.wt,1:5,sselq,"*"),2:6,sum) * 1e-4
@
...
hcr <- function(catch,index,year) {
  ...
  CTAR <- 0.9 * as.numeric(refpts(gen1)[,"yield"][4])             # target catch
  ITAR <- 0.9 * as.numeric(refpts(gen1)[,"biomass"][4]) * 1e-4    # target biomass
  ILIM <- 0                                                       # limit biomass
  
  # perfectly known biomass and catchability
  GB <- as.numeric(quantSums(stock.n(stk2)[,year-1] * stock.wt(stk2)[,year-1] * catch.sel(gen1))) * 1e-4 
...
for(y in (proj_strt+1):proj_end) {
  ...
  index[,y] <- apply(sweep(stk2@stock.n[,y] * exp(-stk2@m[,y]/2) * stk2@stock.wt[,y],1:5,sselq[,y],"*"),2:6,sum) * 1e-4
}
@
...
hcr <- function(catch,index,year) {
  ...
  CTAR <- 0.9 * as.numeric(refpts(gen1)[,"yield"][4])             # target catch
  ITAR <- 0.9 * as.numeric(refpts(gen1)[,"biomass"][4]) * 1e-4    # target cpue
  ILIM <- 0                                                       # limit cpue
  
  # perfectly known biomass
