...
using the using the FLR library~\cite{FLRref}.
...
other also uses C code. The version that uses C code is considerably faster and
...
the year. The Edwards model assumes that the fishing mortality happens
...
in the model parameters. For example, Francis estimates biomass and yield
for a range of values of natural mortality. This approach can be applied.
