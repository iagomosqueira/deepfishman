...
%\begin{shaded}%
<<echo=TRUE>>=
exp.biomass.mid(or,yrfrac = 0.5,virgin=TRUE)
@
%\end{shaded}%
\end{minipage}
\end{center}
...
Francis (REF) provides a Monte Carlo routine for estimating a probability
...
Francis(REF) the procedure is as follows:
1) Choose a trial value of $B_{0}$.
2) Calculate the biomass history for this trial $B_{0}$.
3) Generate simulated indices by assuming that the indices are normally
...
4) Calculate the value of $B_{0sim}$ that maximises the likelihood estimate
for the simulated survey indices.
5) Repeat steps 1-4 100 times and calculate the proportion of the $B_{0sim}$
estimates that are greater than $B_{0}$HAT.
6) Repeat steps 1-5 for a range of trial of $B_{0}$ values.

This process is relatively easy to carry out using $FLaspm$ because multiple
iterations can be processed in parallel.
It should be remembered that here
$B_{0}$ is the virgin biomass at the start of the year whilst the index is
the relative abundance midyear.
A sequence of trial $B_{0mid}$ values and corresponding $B_{0trial}$ values at
...
likelihood for each iteration is calculated. The probability of this value
of $B_{0sim}$ being greater than $B_{0trial}$ is calculated.
The R code for this procedure using the New Zealand Orange Roughy
data is shown below:
