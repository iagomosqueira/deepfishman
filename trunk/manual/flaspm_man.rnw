...
This package implements two age-strucutured production models based on
~\cite{Francis92} and~\cite{Walters06},
using the FLR library~\cite{FLRref}.

Age structure population models track the number of fish per age cohort forward over time. 
The models implemented here describe a relatively simple parameterisation known as
a stock reduction model. All life history parameters
(specifically growth, age specific natural mortality and recruitment) are assumed known with associated
uncertainty. Commerical catch and selectivity is assumed complete and known without error. For both the Francis and Walters
models the intention is to estimate the spawning biomass at equilibrium from one or more
indices of abundance, so that the stock trajectory can be reconstructed using the catch series and selectivity.   
...
The models also differ in the parameters estimated. The Francis model only
estimates $B_0$ ($\sigma^2$ and $\hat{q}$ are approximated) whereas the Walters model 
estimates both $B_0$ and $\sigma^2$ ($\hat{q}$ is still approximated).

Statistical uncertainty is dealt with differently too. MORE HERE.

% B0, q and F for francis
% B0, q and sigma2 for walters

% and how statiscal uncertainty in these parameters is dealt with
% (think we should probably mention this if we are going to submit
% to a stats journal!)
...
In addition to $B0$, the model also estimates the observation uncertainty around 
the abundance ($\sigma^2$).

\subsection{Fitting the model}

For both models $FLaspm$ uses the $fmle()$ method to estimate the
virgin exploitable biomass. Likelihood is described as a log-normal
distribution around the abundance index.
...
The fitting is carried out in a two stage process.
First, a simulated annealing method may be used. The number of iterations performed
is controlled by the control argument $SANN_maxit$ (see below for an example).
By default this is set to zero iterations and simulated annealing is not performed.
Using simulated annealing may take a long time, particularly when
the pure R version of the models are used. The second stage uses a quasi-Newton method (BFGS).
Convergence criteria for this stage can be set by the user (see the help file
for $optim$ in R for control parameters). By default the BFGS method is bounded
(the optim method $L-BFGS-B$). However, this method is unable to cope with values that are not
finite. The likelihood function for the Francis model is undefined for cases
where the population goes extinct through fishing pressure. For this reason
the unbounded $BFGS$ method must be used for this model.
...
at age and the fishing mortality (or harvest rate for the Edwards model).
...
($amin$ and $amax$) the $catch$, the biomass $indices$,
...
dimnames(catch)
dimnames(index)
# Note that index has NA in some years but number of years is same as catch
...
Additionally, the C version uses automatic differentiation (REF ADOLC) to
accurately calculate the gradients used by the solver. This assists the solver.
...
setting. To estimate the parameters use the $fmle()$ method. As mentioned above
for the Francis model we need to specify that the unbounded BFGS method is used.
In this example, we also do not use simmulated annealing (the default number of
iterations for the $SANN$ solver is 0).

\begin{center}
\begin{minipage}[H]{0.95\textwidth}%
\begin{shaded}%
<<echo=TRUE,keep.source=TRUE,results=hide>>=
or <- fmle(or,method="BFGS")
@
\end{shaded}%
\end{minipage}
\end{center}
...
parameter value. You should see that the value of the fitted parameter (the dot
on the plot) occurs at the
...
% Produces the plot and captions it
\begin{figure}[!hb]
\begin{center}
%\begin{shaded}%
...
estimate is pretty good. We can also check out estimated value of $\hat{q}$ with
...
Francis reports the estimated value of $\hat{q}$ to be 0.59~\cite{Francis92}.
...
history and with a CV of 15\% (estimated by Francis).
...
The history of biomass in the middle of the year for each
...
to generate all 100 iterations simultaneously.
The simulated index should only have data in the years which the original index series had data.
A new $FLaspm$ object is set up
...
data is shown below. This may take a little while to run.
...
# Set up the years for which is there should be no index data
na.years <- dimnames(index)$year[which(is.na(index))]
# Loop over each value of B0
for (i in 1:length(B0trial))
{
  ...
  # Set the appropriate index years to NA
  ...
  or.sim <- fmle(or.sim,method="BFGS")
...
cab then be calculated and plotted (see Figure~\ref{fig:or_probB0_OR}).
...
@
\end{center}
\caption{Estimates of the probability that the true $B_{0}$, is less than
...
To allow comparison with Francis we use the midyear biomasses here (the
...
plot(Bmidplot,jp,type="l",xlab="Mid year biomass (000 t)", ylab="Probability")
@
\end{shaded}%
\end{minipage}
\end{center}
...
@
\end{center}
\caption{Estimated probability density function of $B_{0}$}
\label{fig:or_pdfB0_OR}
\end{figure}

The plotted PDF is equivalent to that published by Francis.
...
in future recruitment. (NOTE: given that the age of selectivity is 23,
I do not see how variable recruitment will impact the fishery within the
...
for each schedule. This may take a little while.
...
for (schedule in 1:dim(TAC_schedules)[1])
{
  ...
  or <- FLaspm(catch=catch_proj, index=index_proj, M=M,hh=hh,sel=sel_age,
              mat=mat_age, wght=w, amax=amax, amin=amin,sr_res=sr_res)
  model(or) <- aspm.Francis.CAD()
  # Set up B0 values from previously fitted Johnson distribution
  # Remember we scaled B0 by 1000 for the fit and that the probability
  # distribution was for Bmid
  params(or) <- propagate(params(or),iters)
  Bmid <- rJohnson(iters,jcpars)*1000
  params(or)["B0",] <- Bmid*exp(0.5*M)
  # Project forward
...
\end{minipage}
\end{center}
% save(prop_collapse,file="~/Work/deepfishman/trunk/manual/prop_collapse.RData")
...
Now we can plot the probability of collapse against the rate of TAC reduction.
\begin{center}
\begin{minipage}[H]{0.95\textwidth}%
...
We can check the model estimates of $B0$ and $\sigma^2$ using a two
