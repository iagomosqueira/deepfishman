...
# access sr parameters
sr_model <- model(gen1)
sr_params <- params(gen1)
...
for(e in 1:length(eff_seq)) {
  ...
  for(i in 1:nit) {
      cat("\ti: ", i, "\n")
    index.iter <- iter(index,i)
    stk_proj.iter <- iter(stk_proj,i)
    sr_resid.iter <- iter(sr_resid,i)
    
    for(y in (proj_strt+1):proj_end) {
      cat("\t\ty: ", y, "\n")
      # control rule
      ctrl <- hcr(catch(stk_proj.iter),index.iter,y)
      # project
      # Problem with iters here - residuals needs to have dimnames starting at 1
      dimnames(sr_resid.iter)$iter <- "1"
      stk_proj.iter <- fwd(stk_proj.iter,ctrl=ctrl,sr = list(model=sr_model,params=sr_params),sr.residuals=sr_resid.iter)
    # exploitable biomassx`
      index.iter[,y] <- quantSums(stock.n(stk_proj.iter)[,y] * stock.wt(stk_proj.iter)[,y] * catch.sel(gen1)) 
    }
    
    iter(index,i) <- index.iter
    iter(stk_proj,i) <- stk_proj.iter
  }
