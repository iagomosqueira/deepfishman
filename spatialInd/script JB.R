...
dyn <- function(N,r_star,F) {
 ...
 # calcuate migration coefficients
  diff<- vector('numeric',length=dim(N)[5])
 
 #diff between realised suitabilities, neg. will move out , positive will move in
 
 diff[1]<- (r_star[,,,,2] - r_star[,,,,1]) # this is the spatial gradient in realised suitability between the two areas
 diff[2]<- (r_star[,,,,1] - r_star[,,,,2]) 
...
#  }
#
  N[,,,,1] <- max(N[,,,,1] + r_star[,,,,1]*N[,,,,1] - F[,,,,1]*N[,,,,1] - diff[1]*N[,,,,1],0) #change sign
  N[,,,,2] <- max(N[,,,,2] + r_star[,,,,2]*N[,,,,2] - F[,,,,2]*N[,,,,2] - diff[2]*N[,,,,2],0)
...
N[,1] <- c(1,10) # start with different initial values #r/b
